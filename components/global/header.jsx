...
const Links = () => {
  ...
  const pagerefs = useRefs()
  
  const router = useRouter()

  const handleClick = async (ref) => {
    router.asPath != '/' && await router.push('/')
  ...
  return (
    <ul className='flex gap-x-10 items-center max-md:hidden'>
      <li className='text-2xl'>Arsi</li>
      <li className='cursor-pointer' onClick={() => handleClick(pagerefs.servicesRef)}>Services</li>
      <li className='cursor-pointer' onClick={() => handleClick(pagerefs.projectsRef)}>Projects</li>
      <li className='cursor-pointer' onClick={() => handleClick(pagerefs.aboutRef)}>About me</li>
